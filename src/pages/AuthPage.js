...
export default function AuthPage() {
  const { signIn, signUp, isConfigured: authConfigured, isPasswordRecovery } = useAuth();
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-2xl shadow-card border border-slate-200 p-7">
          ...
          {/* ── Password recovery form (replaces everything when active) ── */}
          {isPasswordRecovery && (
            <div>
              <h2 className="text-lg font-bold text-slate-900 mb-1">Set a new password</h2>
              <p className="text-sm text-slate-500 mb-6">You're all verified — just pick a new password and you're good to go.</p>

              {passwordUpdated ? (
                <div className="flex items-center gap-2 p-4 bg-emerald-50 border border-emerald-200 rounded-lg text-emerald-700 text-sm font-semibold">
                ...
                <form onSubmit={handlePasswordUpdate} className="space-y-4">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-9 pr-3.5 py-2.5 rounded-lg border border-slate-300 bg-white text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition text-sm"
                      />
                    </div>
                  </div>
                  <div>
                  ...
                  <button type="submit" disabled={loading} className="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-brand-600 text-white font-semibold text-sm hover:bg-brand-700 disabled:opacity-50 transition">
                    {loading ? 'Updating…' : 'Set new password'} {!loading && <ChevronRight size={16} />}
                  </button>
                </form>
              )}
            </div>
